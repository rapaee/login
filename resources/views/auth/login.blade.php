    ...
    <form method="POST" action="{{ route('login') }}" >
        ...
        <div class=" flex justify-between mt-4">
            ...
            <a class="text-sm text-gray-600 hover:text-gray-900 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500" href="{{ route('register') }}">
                {{ __('Belum Punya Akun? Registrasi') }}
            </a>
        @endif

        </div>

       <div class="flex justify-center mt-4 py-6 ">
        <x-primary-button class="hover:bg-gray-600 text-white py-2 px-4 rounded-md text-center w-full">
            {{ __('Log in') }}
        </x-primary-button>
        </div>
